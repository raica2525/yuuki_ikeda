...
HRESULT CResult::Init(void)
{
	...
	D3DXCreateFont(CManager::GetRenderer()->GetDevice(), 100, 0, 0, 0, FALSE, SHIFTJIS_CHARSET,
...
void CResult::UpdateRanking(void)
{
	...
	{
		fscanf(pFile, "%d", &m_nTopScore[nCount]);
	}
	fclose(pFile);
	...
	if (m_nTopScore[MAX_RANKING - 1] <= Score)
	{
		...
		m_Rank--;
		for (; m_nTopScore[m_Rank - 1] <= m_nTopScore[m_Rank] ; m_Rank--)
		{
			if (m_Rank <= 0)
			{
				break;
			}
			int Buff = m_nTopScore[m_Rank - 1];
			m_nTopScore[m_Rank - 1] = m_nTopScore[m_Rank];
			m_nTopScore[m_Rank] = Buff;
		}
	}
...
void CResult::DrawRanking(void)
{
	...
	if (m_Rank < MAX_RANKING)
	{
		sprintf(cNum, "あなたは%d位", m_Rank+1);
		strcat(str, cNum);
	}
	else
	{
		sprintf(cNum, "あなたはランク外", m_Rank);
		strcat(str, cNum);
	}
	//テキスト描画
	m_pFont->DrawText(NULL, str, -1, &rect, DT_CENTER|DT_VCENTER, D3DCOLOR_ARGB(0xff, 0xff, 0xff, 0xff));
}