...
void CManager::Update(void)
{
	...
	if (CMap::GetGoal())
	{
		...
		{
			CEnemy::Create(D3DXVECTOR3(SCREEN_WIDTH, (rand() % 7 + 2.5)*MAP_TILE_SIZE, 0), (CEnemy::TYPE)(rand() % (CEnemy::TYPE_SPRINKLER + 1)));
		}
	}
	...
	if (((CInputKeyboard*)m_pInputKey)->GetKeyTrigger(DIK_P))
	{
		...
		CEnemy::Create(D3DXVECTOR3(point.x, point.y, 0),CEnemy::TYPE_NORMAL);
	}
	else if (((CInputKeyboard*)m_pInputKey)->GetKeyTrigger(DIK_O))
	{
		...
		GetCursorPos(&point);
		ScreenToClient(FindWindow(CLASS_NAME, NULL), &point);
		CEnemy::Create(D3DXVECTOR3(point.x, point.y, 0), CEnemy::TYPE_SPRINKLER);
	}
