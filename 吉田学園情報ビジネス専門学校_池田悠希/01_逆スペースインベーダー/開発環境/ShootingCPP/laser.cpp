...
void CLaser::Update(void)
{
    ...
    if(pClosestEnemy != NULL && pClosestTile != NULL || apHitEnemy[0] != NULL && pClosestTile != NULL)
    {
        ...
        if(m_bPenetrate)
        {
            for(int nCount = 0; nCount < nHitEnemyCount; nCount++)
            {
                ...
                if((sqrt(pow(EnemyPos.x - m_pos.x, 2) + pow(EnemyPos.y - m_pos.y, 2.0f))) < (sqrt(pow(TilePos.x - m_pos.x, 2) + pow(TilePos.y - m_pos.y, 2.0f))))
                {
                    apHitEnemy[nCount]->HitBullet(m_nDamage);
                }
                size = D3DXVECTOR3(sqrtf(powf(TilePos.x - m_pos.x, 2.0f) + powf(TilePos.y - m_pos.y, 2.0f)), (float)m_nWidth, 0.0f);
        ...
        else
        {
            ...
            if((sqrtf(powf(EnemyPos.x - m_pos.x, 2.0f) + powf(EnemyPos.y - m_pos.y, 2.0f))) < (sqrtf(powf(TilePos.x - m_pos.x, 2.0f) + powf(TilePos.y - m_pos.y, 2.0f))))
            {
                pClosestEnemy->HitBullet(m_nDamage);
                size = D3DXVECTOR3(sqrtf(powf(EnemyPos.x - m_pos.x, 2.0f) + powf(EnemyPos.y - m_pos.y, 2.0f)), (float)m_nWidth, 0.0f);
    ...
    {
        D3DXVECTOR3 TilePos = pClosestTile->GetPos();
        D3DXVECTOR3 size    = D3DXVECTOR3(sqrtf(powf(TilePos.x - m_pos.x, 2.0f) + powf(TilePos.y - m_pos.y, 2.0f)), (float)m_nWidth, 0.0f);
...
void CLaser::Create(D3DXVECTOR3 pos, D3DCOLOR col, int nWidth, float fRot, int nDamage, bool bPenetrate)
{
    ...
    pLaser->m_pPolygon    = CPolygon::Create(D3DXVECTOR3((float)(pLaser->m_pos.x + ((SCREEN_WIDTH * 1.5f) / 2)), pLaser->m_pos.y, 0.0f), D3DXVECTOR3(SCREEN_WIDTH * 1.5, (float)pLaser->m_nWidth, 0), pLaser->m_fRot, OBJTYPE_LASER);
    pLaser->m_pLaserLayer = CLaserLayer::Create(D3DXVECTOR3((float)(pLaser->m_pos.x + ((SCREEN_WIDTH * 1.5f) / 2)), pLaser->m_pos.y, 0.0f), pLaser->m_col, pLaser->m_nWidth, (int)(SCREEN_WIDTH * 1.5f), pLaser->m_fRot);
}
