...
public:
    ...
    HRESULT StartEffect(int nController, int nFrame);
    HRESULT StopEffect(int nController);
    static HRESULT CALLBACK EnumObjectCallBack(const LPCDIDEVICEOBJECTINSTANCE lpddoi, LPVOID pvRef);
private:
    static CONTROLLER m_aController[MAX_PLAYER];    // コントローラーの情報	
    static int m_nCntController;					// コントローラーの接続数
    LPDIRECTINPUTEFFECT m_aEffectInput[MAX_PLAYER];	// エフェクトのポインタ
    int m_anFrameEffect[MAX_PLAYER];				// 振動する残りフレーム
    DWORD m_adwNumFFBAxis[MAX_PLAYER];				// 振動の軸の数
};
