...
//
// 入力処理 (input.cpp)
// Author : 後藤慎之助、池田悠希（コントローラの振動、再接続対応）
//
//====================================================================
...
CInputJoypad::CONTROLLER CInputJoypad::m_aController[MAX_PLAYER] = {};
...
HRESULT CInputJoypad::Init(HINSTANCE hInstance, HWND hWnd)
{
    ...
    // コントローラーを探す
    hr = m_pInput->EnumDevices(DI8DEVCLASS_GAMECTRL, (LPDIENUMDEVICESCALLBACK)GetJoystickCallback, this, DIEDFL_ATTACHEDONLY); //すでに接続されていてフォースフィードバック対応しているコントローラーを列挙
...
void CInputJoypad::Release(void)
{
    for (int nCntDevice = 0; nCntDevice < MAX_PLAYER; nCntDevice++)
    {
        ...
        if (m_aEffectInput[nCntDevice] != NULL)
        {
