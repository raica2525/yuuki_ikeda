...
CSound::PARAM CSound::g_aParam[MAX] = {
  {"Resources\\shot.wav", 0},    // 弾発射音
  {"Resources\\explosion.wav", 0},
  {"Resources\\BGM.wav", XAUDIO2_LOOP_INFINITE},
};
...
HRESULT CSound::Init(HWND hWnd)
{
    ...
    for(int nCntSound = 0; nCntSound < MAX; nCntSound++)
    {
        ...
        // サウンドデータファイルの生成
        hFile = CreateFile(g_aParam[nCntSound].pFilename,
                           GENERIC_READ,
                           FILE_SHARE_READ,
                           NULL,
                           OPEN_EXISTING,
        ...
        hr =
          ReadChunkData(hFile, m_apDataAudio[nCntSound], m_aSizeAudio[nCntSound], dwChunkPosition);
        if(FAILED(hr))
        {
