...
void CLightEnemy::Update(void)
{
	...
	if (GetState() != STATE_DEATH)
	{
		...
		case ENEMY_FOUND:	//プレイヤー発見
			...
			//攻撃アニメーション
			m_pAnima->SetAnimation(CAnimation::MOTION_LIGHT_ENEMY_ATTACK);
	...
	case STATE_DAMAGE:		//ダメージ
		...
		break;
	case STATE_DEATH:		//破壊
		if (m_pAnima->GetAnimeEnd() == false&& m_pAnima->GetAnimation()== CAnimation::MOTION_LIGHT_ENEMY_DEATH)
		{
			//爆発
		...
		if (m_pAnima->GetAnimation() != CAnimation::MOTION_LIGHT_ENEMY_DEATH)
		{
			m_pAnima->SetAnimation(CAnimation::MOTION_LIGHT_ENEMY_DEATH);
		}
		break;
	default:
		break;
