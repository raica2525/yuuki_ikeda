...
void CHeavyEnemy::Update(void)
{
	...
	if (GetState() != STATE_DEATH)
	{
		...
		case ENEMY_TOUR:
			...
			{
				//歩くアニメーションセット
				m_pAnima->SetAnimation(CAnimation::MOTION_LIGHT_ENEMY_MOVE);
		...
		case ENEMY_WARY:
			...
			{
				//クエスチョンマーク生成
				SetQuestion(CQuestion::Create(D3DXVECTOR3(GetPos().x, GetPos().y + 85, GetPos().z), D3DXVECTOR3(0.0f, 0.0f, 0.0f), D3DXVECTOR3(10.0f, 10.0f, 10.0f)));
				//使っている処理
				m_bWary = true;
			}
			//アニメーション立ち止まる
		...
		case ENEMY_FOUND:	//プレイヤー発見
			...
			//攻撃アニメーション
			m_pAnima->SetAnimation(CAnimation::MOTION_HEAVY_ENEMY_ATTACK);
	...
	case STATE_DEATH:		//破壊
		...
		if (m_pAnima->GetAnimeEnd() == false && m_pAnima->GetAnimation() == CAnimation::MOTION_HEAVY_ENEMY_DEATH)
		{
			//爆発
		...
		//アニメーション再生
		if (m_pAnima->GetAnimation() != CAnimation::MOTION_HEAVY_ENEMY_DEATH)
		{
			m_pAnima->SetAnimation(CAnimation::MOTION_HEAVY_ENEMY_DEATH);
		}
		break;
	default:
		break;
